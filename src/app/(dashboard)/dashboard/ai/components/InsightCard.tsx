/**
 * Componente de Card de Insight
 * 
 * FASE 12: Inteligência Artificial Aplicada
 * 
 * Client Component para exibir insights gerados pela IA.
 */

'use client';

import type { FinancialInsight } from '@/lib/ai/types';

interface InsightCardProps {
  insight: FinancialInsight;
}

export default function InsightCard({ insight }: InsightCardProps) {
  const typeColors = {
    positive: {
      bg: 'bg-green-950/50',
      border: 'border-green-800',
      icon: 'text-green-400',
      title: 'text-green-300',
    },
    warning: {
      bg: 'bg-yellow-950/50',
      border: 'border-yellow-800',
      icon: 'text-yellow-400',
      title: 'text-yellow-300',
    },
    info: {
      bg: 'bg-blue-950/50',
      border: 'border-blue-800',
      icon: 'text-blue-400',
      title: 'text-blue-300',
    },
    negative: {
      bg: 'bg-red-950/50',
      border: 'border-red-800',
      icon: 'text-red-400',
      title: 'text-red-300',
    },
  };

  const colors = typeColors[insight.type];

  const icons = {
    positive: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
      </svg>
    ),
    warning: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
        <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
      </svg>
    ),
    info: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
      </svg>
    ),
    negative: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
      </svg>
    ),
  };

  return (
    <div className={`${colors.bg} ${colors.border} border rounded-xl p-6`}>
      <div className="flex items-start gap-4">
        <div className={`${colors.icon} flex-shrink-0`}>
          {icons[insight.type]}
        </div>
        <div className="flex-1">
          <h3 className={`${colors.title} font-semibold text-lg mb-2`}>
            {insight.title}
          </h3>
          <p className="text-slate-300 text-sm mb-3">
            {insight.description}
          </p>
          {insight.recommendation && (
            <div className="bg-slate-900/50 rounded-lg p-3 mt-3">
              <p className="text-xs font-semibold text-slate-400 mb-1">Recomendação:</p>
              <p className="text-slate-300 text-sm">{insight.recommendation}</p>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
